...
int stmvl53lx_reg_wr(int fd, int index, int len , void *data){
...
int stmvl53lx_reg_rd(int fd, int index, int len , void *data){
	struct stmvl53lx_register reg;
	int rc;
	reg.is_read = 1;
	reg.index = index;
...
//	int data_ok;
	...
	for( sz=1; sz<256; sz*=2){
		for( n=0; n<n_loops; n++ ){
			timer_start(&tstart);
			rc = stmvl53lx_reg_rd(fd, 0x0, sz, rd_data);
			if( rc ){
				break;
			}
...
{
	int i;
	fprintf(stderr,	"Usage: vl53lx_iotest test_mask\n");
