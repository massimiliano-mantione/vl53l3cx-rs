...
static int stmvl53lx_reg_wr(int fd, int index, int len , void *data){
	...
	rc= ioctl(fd, VL53LX_IOCTL_REGISTER,&reg);
	if( rc == 0 ){
		return reg.status;
	}
	return -1;
...
static int stmvl53lx_reg_rd(int fd, int index, int len , void *data){
	struct stmvl53lx_register reg;
	int rc;
	reg.is_read = 1;
	reg.index = index;
...
void reg_test_random(int fd){
	...
	rc = stmvl53lx_reg_wr(fd, random_wr_index, 0, rd_data);
	tfail( rc == 0 ," wr 0 length must fail");

	rc = stmvl53lx_reg_rd(fd, random_wr_index, 0, rd_data);
	tfail( rc == 0 ," rd 0 length must fail");

	memset(wr_data, 0xFF, sizeof(wr_data));
	for( sz = 1 ; sz < random_sz; sz++ ){
		rc = stmvl53lx_reg_wr(fd, random_wr_index, sz, wr_data);
		tfail(rc !=0 , "wr sz %d", sz );
		if( rc )
			continue;

		rc = stmvl53lx_reg_rd(fd, random_wr_index, sz, rd_data);
	...
	for( sz=1; sz <= 4; sz*=2){
		for( offset = 0 ; offset <= random_sz/2; offset++ ){
			...
			rc = stmvl53lx_reg_wr(fd, random_wr_index+offset, sz, &wr_data[offset]);
			tfail(rc !=0 , "off wr%d sz %d", offset , sz );
			if( rc )
				break;
			// read back full chunk
			rc = stmvl53lx_reg_rd(fd, random_wr_index, sizeof(wr_data), rd_data);
			tfail(rc !=0 , "off wr %d sz %d", offset, sz );
			if( rc )
				break;
...
{
	fprintf(stderr,
		"Usage: vl53lx_reg REG_ADDR REG_BYTES [REG_DATA]\n"
		" REG_ADDR is VL53LX register address using 0xxx format: \n"
...
int main(int argc, char *argv[])
{
	...
	fd = open("/dev/" VL53LX_MISC_DEV_NAME ,O_RDWR );
	if (fd <= 0) {
		fprintf(stderr,"Error open %s device: %s\n", VL53LX_MISC_DEV_NAME, strerror(errno));
		return -1;
	}
	...
	if (argc == 3) {
		reg.is_read = 1;
		printf("To read VL53LX register index:0x%x, bytes:%d\n", reg.index, reg.cnt);
