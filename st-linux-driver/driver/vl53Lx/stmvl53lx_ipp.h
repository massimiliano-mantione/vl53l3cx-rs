...
#ifndef _STMVL53LX_IPP_H_
#define _STMVL53LX_IPP_H_
 ...
 */
enum stmvl53lx_ipp_proccesing_e {
	stmvl53lx_ipp_ping = 0,
	/*!< stmvl53lx_ipp_ping
	 * @li can be sent by driver to check client is alive
	 * @li  daemon sent it to identify and register himself to the driver
	 */
	stmvl53lx_ipp_cal_hist = 1,
	/*!< stmvl53lx_ipp_cal_hist process cal hist*/

	stmvl53lx_ipp_xtalk_calibration = 2,
	/*!< stmvl53lx_ipp_xtalk_calibration process crosstalk calibration data
	 */

	stmvl53lx_ipp_hist_ambient_dmax = 3,
	/*!< stmvl53lx_ipp_hist_ambient_dmax process ambient dmac calculation
	 * from histogram
	 */

	/** keep last*/
	stmvl53lx_ipp_max /*!< stmvl53lx_ipp_max */
};

/**
 * status use on @a ipp_work_t::status
 */
enum stmvl53lx_ipp_status_e {
	stmvl53lx_ipp_status_ok = 0,	/*!< ok work done */
	stmvl53lx_ipp_status_inv_id,	/*!< dev id  not supported or invalid */
	stmvl53lx_ipp_status_inv_proc,
	/*!< process_no asked  not supported or not implemented */
	stmvl53lx_ipp_status_inv_payload,
	/*!< data payload for asked processing incorrect*/

	stmvl53lx_ipp_status_proc_code = 0x100,
...
struct ipp_work_t {
	...
	/*!< Identify the work do be run see @a stmvl53lx_ipp_proccesing_e */
	uint8_t process_no;
		...
		int max_dev)
{
	...
	IPP_PRINT("process #%d (%s)\n", pw->process_no,
			pw->process_no < stmvl53lx_ipp_max ? "ok" : "bad");
	IPP_PRINT("status %d\n",  pw->status);
