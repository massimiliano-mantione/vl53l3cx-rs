...
/**
 * @file stmvl53lx_i2c.c  vl53lx linux native i2c interface
 *
 */
#include "stmvl53lx.h"
...
/**
 * compute elapsed time in in micro  sec based on st_gettimeofday
...
#	define i2c_debug(fmt, ...) vl53lx_dbgmsg(fmt, ##__VA_ARGS__)
#else
...
/**
 * compute elapsed time in in milli sec based on st_gettimeofday
 * @param tv pointer to start time_val
...
static int cci_write(struct stmvl53lx_data *dev, int index,
...
static int cci_read(struct stmvl53lx_data *dev, int index,
		uint8_t *data, uint16_t len)
{
	uint8_t buffer[2];
...
VL53LX_Error VL53LX_WrByte(VL53LX_DEV pdev, uint16_t index, uint8_t data)
{
	...
	dev = (struct stmvl53lx_data *)container_of(pdev,
...
VL53LX_Error VL53LX_WrWord(VL53LX_DEV pdev, uint16_t index, uint16_t data)
{
	...
	/* Split 16-bit word into MS and L*  stmvl53lx FlightSense sensor */
...
VL53LX_Error VL53LX_WriteMulti(VL53LX_DEV pdev, uint16_t index,
...
VL53LX_Error VL53LX_ReadMulti(VL53LX_DEV pdev, uint16_t index,
		uint8_t *pdata, uint32_t count)
{
	struct stmvl53lx_data *dev;
...
VL53LX_Error VL53LX_WaitValueMaskEx(VL53LX_DEV pdev,
		uint32_t timeout_ms,
		uint16_t index,
		uint8_t value,
		uint8_t mask, uint32_t poll_delay_ms)
{
	...
	do {
		...
		vl53lx_dbgmsg("poll @%x %x & %d != %x", index,
				rd_val, mask, value);
...
VL53LX_Error VL53LX_WaitUs(VL53LX_DEV pdev, int32_t wait_us)
{
	...
	data = (struct stmvl53lx_data *)container_of(pdev,
			struct stmvl53lx_data,
			stdev);
